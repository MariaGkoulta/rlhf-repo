...
from reward_unified import RewardModel, train_reward_model_batched
from bald_unified import select_active_pairs, get_bald_scores_for_pairs, get_bald_scores_for_clips, sample_random_pairs, select_active_clips_for_evaluation
...
def sample_evaluative_data(clips, num_samples):
    ...
    evaluative_data, annotated_clips, true_rewards = annotate_evaluative(
        selected_clips, 
...
def run_training(
    ...
    initial_target_points = int(total_target_pairs * initial_collection_fraction)
    print(f"Targeting {initial_target_points} initial total feedback data points.")
    current_min_gap = initial_min_gap
    print(f"Initial feedback generation with MIN_GAP: {current_min_gap}")
    ...
    # Initial data collection: 50% preference, 50% evaluative
    num_initial_prefs = initial_target_points // 2
    num_initial_evals = initial_target_points - num_initial_prefs
    ...
    random.shuffle(initial_prefs)
    val_split_idx_pref = int(len(initial_prefs) * 0.2)
    val_prefs = initial_prefs[:val_split_idx_pref]
    train_prefs = initial_prefs[val_split_idx_pref:]
    ...
    val_eval_data = initial_eval_data[:val_split_idx_eval]
    train_eval_data = initial_eval_data[val_split_idx_eval:]
    ...
    while T_cumulative_ppo_steps_in_loop < total_ppo_timesteps:
        ...
        print(f"Iteration {it}: PPO Timesteps {T_cumulative_ppo_steps_in_loop}/{total_ppo_timesteps}, Total Feedback: {len(train_pref_ds) + len(train_eval_ds)}/{total_target_pairs}")
        ...
        num_iters_left = (total_ppo_timesteps - T_cumulative_ppo_steps_in_loop) / ppo_timesteps_per_iter if ppo_timesteps_per_iter > 0 else 0
        remaining_needed_overall = total_target_pairs - (len(train_pref_ds) + len(train_eval_ds))
        if num_iters_left > 0:
        ...
        if use_random_sampling and target_points_this_iter > 0 and len(clips_ds) > 1:
            print(f"Randomly acquiring {target_points_this_iter} new data points.")
            num_new_prefs = target_points_this_iter // 2
            num_new_evals = target_points_this_iter - num_new_prefs
        ...
        if new_prefs:
            val_split_idx = int(len(new_prefs) * 0.2)
            random.shuffle(new_prefs)
            for c1, c2, p in new_prefs[val_split_idx:]:
                train_pref_ds.add(c1, c2, p)
            for c1, c2, p in new_prefs[:val_split_idx]:
                val_pref_ds.add(c1, c2, p)

        if new_evaluative_data:
            val_split_idx = int(len(new_evaluative_data) * 0.2)
            random.shuffle(new_evaluative_data)
            for clip, rating in new_evaluative_data[val_split_idx:]:
                train_eval_ds.add(clip, rating)
            for clip, rating in new_evaluative_data[:val_split_idx]:
                val_eval_ds.add(clip, rating)
