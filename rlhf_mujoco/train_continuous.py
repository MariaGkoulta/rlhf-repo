...
from torch.utils.tensorboard import SummaryWriter
from rlhf_reacher.configs.cheetah_config import *
import shutil
...
def run_training(
    ...
    initial_policy_ts, total_ppo_timesteps,
    ...
    bald_pair_queue = [] # Queue for pre-selected BALD pairs

    pairs_to_collect_in_loop = total_target_pairs - (len(train_dataset) + len(val_dataset))
    if pairs_to_collect_in_loop > 0:
        ppo_timesteps_per_iter = total_ppo_timesteps // pairs_to_collect_in_loop
    else:
    ...
    while T_cumulative_ppo_steps_in_loop < total_ppo_timesteps:
        ...
        # Invalidate the BALD queue at the start of each major iteration
        # to ensure we use the latest trajectories for selection.
        if use_bald:
            bald_pair_queue.clear()
        ...
        timesteps_to_train = min(ppo_timesteps_per_iter, total_ppo_timesteps - T_cumulative_ppo_steps_in_loop)
        ...
        T_cumulative_ppo_steps_in_loop += timesteps_to_train
        ...
        if (len(train_dataset) + len(val_dataset)) < total_target_pairs:
            ...
            if FEEDBACK_TYPE == "preference" and len(clips_ds) >= 2:
                if use_random_sampling:
                    new_prefs = sample_random_preferences(clips_ds, 1, current_min_gap)
                elif use_bald:
                    # If the queue is empty, populate it with a new batch of high-scoring pairs.
                    # This will happen once per major training iteration due to the clear() above.
                    if not bald_pair_queue:
                        print("BALD pair queue is empty. Selecting a new batch of high-scoring pairs...")
                        # Select a batch of pairs to amortize the cost of BALD calculation
                        cand_pairs = select_active_pairs(
                            clips_ds, reward_model, pool_size=BALD_POOL_SIZE, K=50, T=bald_t,
                            device=device, logger=policy.logger, iteration=reward_logger_iteration, results_dir=results_dir
                        )
                        if cand_pairs:
                            bald_pair_queue.extend(cand_pairs)
                            print(f"Added {len(cand_pairs)} pairs to the BALD queue.")
                    
                    if bald_pair_queue:
                        # Get the next best pair from the queue
                        next_pair = bald_pair_queue.pop(0)
                        annotated_prefs, _, _ = annotate_pairs([next_pair], min_gap=current_min_gap)
                        if annotated_prefs:
                            new_prefs = annotated_prefs
                elif use_ensemble:
                    new_prefs = select_high_variance_pairs(
                        clips_ds, reward_ensemble, 1, current_min_gap,
            ...
            if new_prefs:
                ...
                print("Retraining reward model with new pair...")
                if use_random_sampling or use_bald:
                    reward_model, reward_logger_iteration = train_reward_model_batched(
                        reward_model, train_dataset, val_dataset, device=device, epochs=5, patience=3,
                        optimizer=optimizer, regularization_weight=rm_reg_weight, logger=policy.logger,
                        iteration=reward_logger_iteration, feedback_type=FEEDBACK_TYPE
                    )
                    for sub in vec_env.envs:
                        sub.reward_model = reward_model
                else:
                    reward_ensemble.train_ensemble(
                        train_dataset, val_dataset, device=device, epochs=5, patience=3,
                        optimizer_lr=rm_lr, optimizer_wd=rm_weight_decay,
                        regularization_weight=rm_reg_weight, logger=policy.logger,
                        iteration=reward_logger_iteration, feedback_type=FEEDBACK_TYPE
                    )
                    for sub in vec_env.envs:
        ...
        model_to_save = reward_ensemble if use_ensemble else reward_model
        torch.save(model_to_save.state_dict(), os.path.join(results_dir, f"rm_iter_{it}.pth"))
        plot_correlation_by_bin(clips_ds, model_to_save, it, results_dir, writer=writer)
