...
from preferences import PreferenceDataset, annotate_preferences, clip_return, create_bins, create_preferences
...
NUM_BINS = 20
DEFAULT_SAMPLES_PER_OTHER_BIN = 100
...
clips_ds.extend(init_clips)
bins = create_bins(bins, clips_ds, results_dir=results_dir, iteration=0, num_bins=NUM_BINS)
preference_data, reward_differences, rewards = create_preferences(bins, num_samples_per_other_bin=DEFAULT_SAMPLES_PER_OTHER_BIN, min_gap=MIN_GAP)
...
for iteration in range(1, TOTAL_ITERS):
    ...
    clips    = collect_clips(policy, num_clips=CLIPS_PER_UPDATE, segment_len=SEGMENT_LEN)
    clips_ds.extend(clips)
    bins = create_bins(bins, clips_ds, results_dir=results_dir, iteration=iteration, num_bins=NUM_BINS)
    ...
    new_prefs, reward_differences, prefcollected_rewards = create_preferences(
        bins,
        num_samples_per_other_bin=DEFAULT_SAMPLES_PER_OTHER_BIN,
        min_gap=MIN_GAP
    )
    ...
    all_pref_rewards = [reward for pair in prefcollected_rewards for reward in pair]
    plt.figure(figsize=(10, 6))
    plt.hist(all_pref_rewards, bins=50, color='lightgreen', edgecolor='black')
    plt.title(f"Iteration {iteration} - Distribution of Collected Preference Rewards")
    plt.xlabel("Sum of True Rewards per Clip")
    plt.ylabel("Frequency")
    plt.grid(axis='y', alpha=0.75)
    plot_path_prefs_collected = os.path.join(results_dir, f"iter_{iteration}_pref_collected_rewards_dist.png")
    plt.savefig(plot_path_prefs_collected)
    plt.close()
    print(f"Saved preference collected rewards distribution plot to {plot_path_prefs_collected}")
...
# save reward model
reward_model_path = os.path.join(results_dir, "reward_model" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + ".pth")
torch.save(reward_model.state_dict(), reward_model_path)
print(f"Reward model saved to {reward_model_path}")
