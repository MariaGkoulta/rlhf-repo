...
from reward_model_ensemble import RewardModel
import random
...
class OnlineRLHF:
    ...
    def __init__(self, env_name=CONFIG['env_name'], device=CONFIG['device']):
        ...
        self.reward_model = RewardModel(state_dim=self.state_dim, hidden_dim=CONFIG['hidden_dim']).to(device)
        ...
        for _ in range(ensemble_size):
            model = RewardModel(state_dim=self.state_dim,hidden_dim=CONFIG['hidden_dim']).to(device)
            self.reward_models.append(model)
            optimizer = optim.Adam(model.parameters(), lr=CONFIG['reward_model_lr'])
            self.reward_optimizers.append(optimizer)
    ...
    def compute_trajectory_reward(self, trajectory):
        ...
        for model in self.reward_models:
            rewards = model(states_tensor, actions_tensor)
            reward_sums.append(rewards.sum().item())
        return np.mean(reward_sums)
    
    # Keep a tensor output for backpropagation
    def compute_trajectory_reward_tensor(self, trajectory, model):
        states = [step[0] for step in trajectory]
        actions = [step[1] for step in trajectory]
        states_tensor = torch.tensor(np.array(states), dtype=torch.float32, device=self.device)
        actions_tensor = torch.tensor(np.array(actions), dtype=torch.long, device=self.device)
    ...
    def compute_trajectory_reward_single(self, trajectory, model):
        return self.compute_trajectory_reward_tensor(trajectory, model).item()
    ...
    def calculate_model_uncertainty(self, trajectories):
        """
        For each trajectory, compute a per-step uncertainty as the variance of the 
        reward predictions from all ensemble members. Then, average these variances 
        for the trajectory. Finally, return an overall uncertainty and a sorted list 
        of trajectories (sorted by uncertainty in descending order).
        """
        all_step_variances = []
        trajectories_uncertainty_list = []
        for traj, reward in trajectories:
            step_uncertainties = []
            ...
            for i in range(len(states)):
                state_i = states_tensor[i].unsqueeze(0)
                action_i = actions_tensor[i].unsqueeze(0)
                preds = []
                for model in self.reward_models:
                    pred = model(state_i, action_i)
                    preds.append(pred.item())
                var_i = np.var(preds)
                step_uncertainties.append(var_i)
            avg_traj_uncertainty = np.mean(step_uncertainties) if step_uncertainties else 0.0
            trajectories_uncertainty_list.append((traj, reward, avg_traj_uncertainty))
            all_step_variances.extend(step_uncertainties)
        overall_uncertainty = np.mean(all_step_variances) if all_step_variances else 0.0
        sorted_trajectories = sorted(trajectories_uncertainty_list, key=lambda x: x[2], reverse=True)     
        return overall_uncertainty, sorted_trajectories
    ...
    def calculate_information_gain(self, traj_pair):
        traj1, traj2, _ = traj_pair
        differences = []
        for model in self.reward_models:
            r1 = self.compute_trajectory_reward_single(traj1, model)
            r2 = self.compute_trajectory_reward_single(traj2, model)
    ...
    def sort_pairs_by_information_gain(self, pairs, trajectories, current_uncertainty, n_samples=10, train_epochs=5):
        ...
        for traj_pair in pairs:
            info_gain = self.calculate_information_gain(traj_pair)
            information_gains.append(info_gain)
        plt.figure(figsize=(10, 6))
        plt.plot(information_gains, 'r-o', linewidth=2)
        plt.title('Information Gain (Ensemble Variance)')
        plt.xlabel('Pair Index')
        plt.ylabel('Variance of Ensemble Differences')
        plt.savefig('information_gain.png')
        sorted_pairs = [pair for _, pair in sorted(zip(information_gains, pairs), key=lambda x: x[0], reverse=True)]
        return sorted_pairs
    ...
    def train_reward_model(self, preference_pairs, epochs=5):
        ...
        for epoch in range(epochs):
            ...
            for traj1, traj2, preferred in preference_pairs:
                target = torch.tensor(float(preferred), dtype=torch.float32, device=self.device).unsqueeze(0)
                for model, optimizer in zip(self.reward_models, self.reward_optimizers):
                    optimizer.zero_grad()
                    r1 = self.compute_trajectory_reward_tensor(traj1, model)
                    r2 = self.compute_trajectory_reward_tensor(traj2, model)
                    diff = r1 - r2
                    prob = torch.sigmoid(diff)
                    loss = self.reward_loss(prob.unsqueeze(0), target)
                    loss.backward()
                    optimizer.step()
                    total_loss += loss.item()
            avg_loss = total_loss / (len(preference_pairs) * len(self.reward_models))
    ...
    def train(self, iterations=20, trajectories_per_iter=200, trajectories_to_collect=10, preference_pairs=50, num_candidate_pairs=200,
        ...
        for iter in range(iterations):
            ...
            uncertainty, trajectories_with_uncertainty = self.calculate_model_uncertainty(trajectories)
